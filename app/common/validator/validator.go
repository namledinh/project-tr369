...
func ValidateEndpointID(endpointID, mac string) error {
	...
	prefix := mac[:6]
	expected := fmt.Sprintf("os::%s-%s", prefix, mac)

	if endpointID != expected {
		return fmt.Errorf("invalid endpoint_id: got %s, expected %s", endpointID, expected)
	}
